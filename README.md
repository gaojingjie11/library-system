...
后端每次启动都会自动创建索引（`config/indexes.js`），不需要手工执行 SQL 或脚本：

- `user_name_active_unique`：`user.name` 的**部分唯一索引**（`partialFilterExpression: { del: 0 }`）。只有活跃账号的用户名唯一，并发注册同名账号时只会有一个成功；已被删除的账号不再占用用户名，同名可以重新注册。
- `book_borrowings_user_del`：`book.borrowings.userid + borrowings.del`，用于按用户批量处理未归还记录（例如删除账号时自动归还）。

如果升级前库里已经存在**重名的活跃账号**，唯一索引会创建失败并阻止启动，日志会直接列出重复的用户名，例如：

```
启动失败：存在重复的活跃账号，无法建立用户名唯一索引：reader(2 个)。请先合并或删除多余账号后重启
```

按提示处理掉重复账号后重新启动即可，索引会自动补建。停用旧版借阅集合 `list` 的检查仍会在建索引之前执行。
