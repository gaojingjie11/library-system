...
exports.updateVideoById = async (req, res) => {
  ...
  const result = await database.collection('book').updateOne({ _id: id, del: 0 }, { $set: update });
...
exports.usershow = async (req, res) => {
  ...
  const list = await users.find(query, { projection: { _id: 1, name: 1, head_img: 1, identity: 1 } }).sort({ _id: 1 }).skip(skip).limit(size).toArray();
...
exports.deleteuserById = async (req, res) => {
  const result = await database.collection('user').updateOne({ _id: objectId(req.query.id), del: 0 }, { $set: { del: 1 } });
  if (!result.matchedCount) throw new HttpError(404, '未找到对应的用户');
  ok(res, { message: '删除成功' });
};
